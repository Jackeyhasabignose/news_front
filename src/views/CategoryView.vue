<template>
    ...
    <div class="container">
        <div v-for="category in categories" :key="category.categoryId" class="category-block">
            <!-- 使用 router-link 將類別名稱作為連結，但保持外觀不變 -->
            <input type="checkbox" v-model="category.isSelected">
            <router-link :to="'/findNewsByCategory/' + category.categoryName" class="category-link">
                <!-- <input type="checkbox" v-model="category.isSelected"> -->
                <span class="category-name">{{ category.categoryName }} (最新消息數量: {{ category.newsCount }})</span>
...
export default {
    ...
    created() {
        // 獲取大類別和新聞數量
        this.getAllCategoriesWithNewsCount();
    },
    methods: {
        async getAllCategoriesWithNewsCount() {
            try {
                // 發起獲取大類別數據的請求，假設後端提供了獲取大類別的API
                const response = await axios.get('http://localhost:8080/get_all_categories_with_news_count');
                this.categories = response.data.map(category => ({ ...category, showTitle: false, isSelected: false }));
            } catch (error) {
                console.error('獲取小類別時出錯：', error);
            }
        },
        async confirmDelete() {
            // 找出被勾選的親類別
            const selectedCategories = this.categories.filter(category => category.isSelected);
            ...
            if (categoriesWithZeroNews.length !== selectedCategories.length) {
                alert('最新消息中包含此小類別，無法刪除');
                return;
            }

            // 打印選中的親類別ID數組
            console.log('選中的親類別ID數組:', selectedCategories.map(category => category.categoryId));
