    ...
    <div class="center-container">
        <div>
            ...
            <select v-model="selectedCategoryId" >
                <option v-for="category in parentCategories" :key="category.parentCategoryId"
...
export default {
    ...
    methods: {
        async getAllParentCategoriesWithNewsCount() {
            // 通过GET请求获取大类别列表
            try {
                const response = await this.$axios.get('http://localhost:8080/get_all_parent_categories_with_news_count');
        ...
        async alterParentCategory() {
            ...
            // 获取选中的大类别
            const selectedCategory = this.parentCategories.find(category => category.parentCategoryId === this.selectedCategoryId);

            // 检查新名称是否与旧名称相同
            if (selectedCategory && selectedCategory.parentCategoryName === this.newCategoryName.trim()) {
                alert("新名稱不能跟就名稱重複");
                return;
            }
            ...
            const requestData = {
                ...
                parentCategoryName: this.newCategoryName.trim(),
            };

            try {
                const response = await this.$axios.post('http://localhost:8080/Alter_parent_ategory', requestData);
                // 处理后端响应
